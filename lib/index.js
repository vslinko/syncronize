...
export function synchronizeFunction(fn, config = {}) {
  const {limit = -1, queueKey, onQueueStart, onQueueEnd} = config;
  const autoKey = typeof queueKey !== 'function' && queueKey || `#${index++}`;

  async function processQueue(context, args) {
    const key = typeof queueKey === 'function' ? queueKey(...args) : autoKey;
...
export default function synchronize(config = {}) {
  return function synchronize(obj, prop, descriptor) {
    descriptor.value = synchronizeFunction(descriptor.value, config);
    return descriptor;
  };
}
